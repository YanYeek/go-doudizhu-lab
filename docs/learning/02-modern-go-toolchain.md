...
> 日期：2026-06-12 ｜ 本机 Go 1.26.4（初稿时为 1.25.0，升级记录见第 8 节）
...
本机是 1.26.4，按新语义走。阶段 2 微练习时会亲手验证。
...
## 8. 升级记录：1.25.0 → 1.26.4（2026-06-12，当年 vs 现在补充篇）

| | 当年（GOPATH 时代） | 现在 |
|---|---|---|
| 怎么升级 | 装新版，祈祷旧代码还能编译 | Go 1.x 承诺向后兼容，旧代码直接编译；`go.mod` 的 `go` 行声明项目要求的最低版本 |
| 项目怎么声明版本 | 没有声明机制 | `go mod edit -go 1.26.4` 改一行；工具链比它旧时会报错甚至自动下载新工具链（GOTOOLCHAIN 机制，Go 1.21+） |

本机操作（手动解压式安装对应的升级方式）：

```powershell
# 1. 下载官方 zip：https://go.dev/dl/go1.26.4.windows-amd64.zip
# 2. 旧版改名备份，新版解压到原位置，PATH 不用动
Rename-Item D:\bin\go D:\bin\go-1.25.0-bak
Expand-Archive go1.26.4.windows-amd64.zip -DestinationPath D:\bin
# 3. 项目声明跟进 + 全量验证
go mod edit -go 1.26.4
go build ./... ; go test ./... ; go vet ./...   # 全部通过
```

确认新版稳定后可删除 `D:\bin\go-1.25.0-bak`。
